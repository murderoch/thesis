...
#    for level in config.term.levels:
        ...
        #asdf = level.energy
        #print(asdf)
        #energies.append(asdf)
        #levels.append(config)
        #asdf[0].append(config)
        #asdf[1].append(level.energy)

#asdf = zip(levels, energies) 
#asdf2 = sorted(asdf)
...
#energies = [float(level.energy) for config in Olevels for level in config.term.levels]
#configs = [config for config in Olevels for config in config.term.levels]
#energy = [float(x) for x in energies]

#asdf = sorted(x, key)

#print(Olevels[0].term.maxEnergy)
#Olevels = NIST

#Olevels.sort(key=lambda x: x.term.maxEnergy)


#for x in Olevels:
#    print(x.term.maxEnergy)
    #if type(x) != float:
        #print(type(x))
